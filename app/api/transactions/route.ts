...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (category === 'Salary') {
      const salaryStatus = await connectionPool.query(
        'INSERT INTO transactions (from_account_id,to_account_id, amount, category, description, transaction_type,status) VALUES ($1, $2, $3, $4, $5, $6, $7)',
        [fromAccount, safeToAccount, amount, category, description, 'credit', 'Success']
      );
      ...
      // if(result.)
      return new Response(JSON.stringify({ success: true, message: 'Funds received successfully.' }), { status: 201 });
    }
